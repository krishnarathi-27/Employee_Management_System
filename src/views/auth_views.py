...
class AuthViews:
    ...
    def __init__(self,db_object) -> None:
        ...
        self.obj_auth_controller = AuthControllers(db_object)

    def login(self) -> None:
        """Method that takes user credentials as input"""

        print(PromptsConfig.ATTEMPTS_MESSAGE)
        while True:
            if self.__login_attempts == 0:
                logger.info("Login attempts are exhausted")
                print(PromptsConfig.ATTEMPTS_EXHAUSTED)
                self.__login_attempts = AppConfig.MAX_LOGIN_ATTEMPTS
                time.sleep(20)
                logger.info("Login attempts ressetting")
            else:
                username = input(PromptsConfig.ENTER_USERNAME)
                input_password = maskpass.askpass(PromptsConfig.ENTER_PASSWORD)
                ...
                if not is_valid_success:
                    ...
                    print(PromptsConfig.INVALID_LOGIN.format(self.__login_attempts))
                else:
                    logger.info("Login attempt resetting")
                    self.__login_attempts = AppConfig.MAX_LOGIN_ATTEMPTS

            choice = input(PromptsConfig.EXIT_SYSTEM_PROMPT).lower()
            if choice == "y":
                break
